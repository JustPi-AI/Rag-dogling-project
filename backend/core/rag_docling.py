...
OCR_LANG = "eng+vie"   # OCR tiếng Việt nếu bạn đã cài tessdata vie
...
class RAGDocling:
    ...
    def _ocr_pdf(self, path: str) -> str:
        """OCR toàn bộ PDF khi Docling không đọc được."""
        text_blocks = []
        try:
        ...
        for f in sorted(self.docs_dir.glob("*")):
            ...
            try:
                ...
                if suffix == ".pdf" and fitz is not None:
                    with fitz.open(f) as pdf:
                        for i, page in enumerate(pdf):
                            ...
                            # Nếu trang rỗng → OCR trang đó
                            if not text:
                                print(f"[OCR] Page {page_num} trống, chạy OCR...")
                                try:
                ...
                # === TXT ===
                    ...
                    docs = splitter.create_documents(
                        [text],
                        metadatas=[{"page": 1, "title": f.name, "source": str(f)}]
                    )
                    for d in docs:
                        chunks.append({
                ...
                # === DOCX / PPTX / HTML / fallback PDF (Docling) ===
                else:
                    try:
                    ...
                    # Nếu Docling lỗi + file là PDF → OCR full file
                    if suffix == ".pdf" and d is None:
                        print("[Docling Error] Chuyển sang OCR toàn bộ PDF...")
                        full_text = self._ocr_pdf(str(f))
                        if full_text.strip():
                            docs = splitter.create_documents(
                                [full_text],
                                metadatas=[{"page": 1, "title": f.name, "source": str(f)}]
